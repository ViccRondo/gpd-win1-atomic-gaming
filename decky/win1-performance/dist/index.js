...
function Content() {
    const [level, updateLevel] = SP_REACT.useState(0);
    const [busy, setBusy] = SP_REACT.useState(false);
    SP_REACT.useEffect(() => {
        getLevel().then(updateLevel).catch(() => updateLevel(0));
    }, []);
    const apply = async (nextLevel) => {
        setBusy(true);
        try {
    ...
    return (SP_JSX.jsx(DFL.PanelSection, { title: "\u6E38\u620F\u5185\u6027\u80FD\u663E\u793A", children: SP_JSX.jsx(DFL.PanelSectionRow, { children: SP_JSX.jsx(DFL.DropdownItem, { label: "\u663E\u793A\u7B49\u7EA7", description: busy ? "正在应用…" : "进入游戏后显示；不占用手柄按键", rgOptions: options, selectedOption: level, disabled: busy, onChange: (option) => apply(Number(option.data)) }) }) }));
}
